...
bool QtBCrypt::validatePassword(const QString& inputPassword, const QString& storedHashedPassword)
{
    char* hash = crypt(inputPassword.toLocal8Bit().data(), storedHashedPassword.toLocal8Bit().data());
    return hash && QString::fromLocal8Bit(hash) == storedHashedPassword;
}
